...
describe('authController', () => {
  ...
  describe.only('.facebookAuth', () => {
    it('adds a new user following a POST request to /api/auth/facebook', (done) => {
      User.count()
        .then(oldCount => {
          request(app)
            .post(`/api/auth/facebook?access_token=${process.env.FACEBOOK_ACCESS_TOKEN}`)
            .then((res) => {
              // console.log(res);
              User.count()
                .then(newCount => {
    ...
    it.only('sends back a JWT token if a Facebook access_token is sent via POST to /api/facebook/auth', (done) => {
      request(app)
        .post(`/api/auth/facebook`)
        .send({ access_token: process.env.FACEBOOK_ACCESS_TOKEN })
        // .set('Authorization', process.env.FACEBOOK_ACCESS_TOKEN)
        .then(res => {
          expect(res.body.token).to.exist;
          done();
        })
